...
def check_press_type(index):
    if buttons[index].value == 1:  # Button pushed down
        ...
        if buttons[index].value == 0:
            ...
            if buttons[index].value == 1:  # Second button press
                second_start = time.time()
                buttons[index].wait_for_inactive()
                if buttons[index].value == 0:
                    second_end = time.time()
                    second_press_time = second_end - second_start
                    if second_press_time is not None:
                        if second_press_time < DOUBLE_CLICK_TIME:
                            buttons[index].wait_for_active(timeout=DOUBLE_CLICK_TIME)

                            if buttons[index].value == 1:  # Third button press
                                third_start = time.time()
                                buttons[index].wait_for_inactive()
                                if buttons[index].value == 0:
                                    third_end = time.time()
                                    third_press_time = third_end - third_start
